  ...
  (et! AList<EtDatatypeName~EtDatatypeProps>
    '((Any :args nil :overlap t :predicate (lambda (v) t))
      ...
      (PList
       ...
       :intersect et:type--plist-intersect-args)
...
(defun et:type--plist-intersect-args (args1 args2 intersect _union)
...
(et-defun et:dt--might-overlap-nontrivial? (a-dt: *et-datatype b-dt: *et-datatype) Boolean
  ...
  (or
   ...
   (when-let* ((handler (get name 'et-spec-parse)))
     (if (eq :full (car handler))
         (make-et-repr :generics et:repr--parsing-generics
...
(et-defvar et:repr--test-variables AList<Symbol~*et-var>
  ...
  (let* ((gen-vec (when (vectorp (car args)) (pop args)))
    ...
    (if gen-vec
        (list 'DynFunction (et-parse-matcher in gen-vec et:repr--parsing-generics)
              (et-parse-repr out (append (et--gen-vec-generics gen-vec) et:repr--parsing-generics)))
...
(et:repr--deffactor S:OP op (op-name &rest args)
  :parse (cons op-name
               (if-let* ((op (et:repr--get-op op-name)) (parse (et-op-parse op)))
                   (apply parse args) (et:repr--op-parse-fallback op args)))
  :to-type (let* ((op (et:repr--get-op op-name)))
             (apply (et-op-eval op) (et:repr--op-pre-eval op args)))
  :print  (if-let* ((tostring (et-op-to-string (et:repr--get-op op-name))))
...
(et:repr--defop bind ([var :const] type)
  :to-string (format "{%s : %s}" (et-var-name var) (et:repr--tostring-0 type))
  (make-et-type-case
...
(et:repr--defop infer (type [gen-vec :generics] [matcher :matcher] [yes :repr] [no :repr])
  ...
  (list (et:repr--parse-0 type)
        ...
        (et:repr--parse-0 yes (et--gen-vec-generics gen-vec))
        (et:repr--parse-0 no))
  ...
  (let* ((result (et-infer matcher type yes et:repr--totype-gen-repls)))
...
(et:repr--defop extends? (sub super [yes :repr] [no :repr])
  :to-string (format "{if %s extends %s then %s else %s}"
                     ...
                     (et:repr--tostring-0 yes) (et:repr--tostring-0 no))
  (et:repr--indeterminate-if (et-subtype? sub super) (et:repr--totype-0 yes) (et:repr--totype-0 no)))
...
(et:repr--defop if? (type [yes :repr] [no :repr])
  ...
  (et:repr--indeterminate-if (et-subtype? (et Nil) type) (et:repr--totype-0 no) (et:repr--totype-0 yes)))
...
(et:repr--defop if-nil? (type [yes :repr] [no :repr])
  ...
  (et:repr--indeterminate-if (et-subtype? type (et Nil)) (et:repr--totype-0 yes) (et:repr--totype-0 no)))
      ...
      (let* ((val (et-type-case-value case)))
        (cond
         ((et-datatype-p val)
          ...
          (et:dt-map-type-args (et-datatype-name val) (et-datatype-args val) #'et:type--validate))
         ((et-alias-p val) (mapc #'et:type--validate (et-alias-args val)))
         (t (error "Expected datatype or alias, found %s" val))))

      (dolist (x (et-type-case-binds case))
        (or (and (consp x) (et-var-p (car x)) (et:type--validate (cdr x)))
...
(advice-add #'make-et-type :filter-return #'et:type--validate)
...
(et-defun et:match--validate (matcher: *et-matcher) *et-matcher
...
(advice-add #'make-et-matcher :filter-return #'et:match--validate)
...
(defun et:match--expand-aliases (matcher)
  ...
  (make-et-matcher :repr (et:match--expand-repr-aliases (et-matcher-repr matcher)
  ...
  (cl-loop for case in (et-repr-dnf repr)
           ...
           (cl-loop for factor in case
                    ...
                    (pcase factor
                      (`(S:ALIAS ,name . ,args)
                       (et-repr-dnf
                        (et:match--expand-repr-aliases (et--alias-call name args 'MATCHER scope) scope)))
...
(et-defun et:match--iso-constraints (matcher: *et-matcher type: *et-type) EtMatchResult
  "Entrypoint for calculating constraints."
  (let* ((et:match--constraints-stack nil))
    (et:match--iso-constraints-0 matcher type)))

(et-defun et:match--iso-constraints-0 (matcher: *et-matcher type: *et-type) EtMatchResult
  (et--merge-match-results
...
(et-defun et:match-sub-constraints (matcher: *et-matcher type: *et-type) EtMatchResult
  "Entrypoint for calculating constraints."
  (let* ((et:match--constraints-stack nil))
    (et:match--sub-constraints-0 matcher type)))

(et-defun et:match--sub-constraints-0 (matcher: *et-matcher type: *et-type) EtMatchResult
  (et-stop-recursion et:match--constraints-stack (list 'sub (et-matcher-repr matcher) type)
                     (et--success-match-result nil)
    (et:match--sub-constraints-1 matcher type)))

(et-defun et:match--sub-constraints-1 (matcher: *et-matcher type: *et-type) EtMatchResult
...
(et-defun et:match--sub-constraints-2 (matcher: *et-matcher case: *et-type-case) EtMatchResult
     ...
     (pcase (et-type-case-value case)
       ((and alias (pred et-alias-p))
        (if (not is-super) (et--failed-match-result)
          (et:match--super-constraints-0 (make-et-matcher :generics generics :repr (make-et-repr :generics generics :dnf (list (list match-factor))))
...
(et-defun et:match--super-constraints-0 (matcher: *et-matcher type: *et-type) EtMatchResult
  ;; The 'super' version of scoped constraint checking
  (et-stop-recursion et:match--constraints-stack (list 'super (et-matcher-repr matcher) type)
                     (et--success-match-result nil)
    (et:match--super-constraints-1 matcher type)))

(et-defun et:match--super-constraints-1 (matcher: *et-matcher type: *et-type) EtMatchResult
  (setq matcher (et:match--expand-aliases matcher))

  (cl-loop for m-case in (et-repr-dnf (et-matcher-repr matcher))
           collect (et:match--super-constraints-2 m-case type (et-matcher-generics matcher))
           into results
           finally return (apply #'et--merge-match-results results)))
...
(et-defun et-pp-matcher (matcher: *et-matcher) String
...
(et-defun et:algebra--dt-subtype? (sub: *et-datatype super: *et-datatype) Boolean
  (et-match-result-success (et:algebra--datatype-subtype? sub super)))

(et-defun et:algebra--datatype-subtype? (sub: *et-datatype super: *et-datatype)
          EtSubtypeResult
  (et:dt-constraints
...
(et-defun et:algebra--binds-subtype? (sub-binds: EtBinds super-binds: EtBinds) EtSubtypeResult
...
(et-defun et:algebra--case-subtype? (sub: *et-type-case super: *et-type-case) EtSubtypeResult
  (let* ((result
          (et--merge-match-results
           ...
           (et:algebra--datatype-subtype? (et-type-case-value sub) (et-type-case-value super))
           (et:algebra--binds-subtype? (et-type-case-binds sub) (et-type-case-binds super))))
  ...
  (let* ((et:algebra--subtype-stack nil)
    ...
    (when (et-match-result-success result)
      (if-let* ((inds (et-match-result-value result)))
          (prog1 nil
            (when (listp et:repr--totype-indeterminates)
              (cl-callf append et:repr--totype-indeterminates inds)))
        t))))

(et-defun et--subtype?-0 (sub: *et-type super: *et-type) EtSubtypeResult
...
(et-defun et--subtype?-1 (sub: *et-type super: *et-type) EtSubtypeResult
  ...
  (cl-loop for sub-case in (et-type-cases sub)
           ...
           (cl-loop for super-case in (et-type-cases super)
                    collect (et:algebra--case-subtype? sub-case super-case) into or-results
...
(defun et--subsect (&rest types) (apply #'et:algebra--intersect t types))
(defun et--supersect (&rest types) (apply #'et:algebra--intersect nil types))
...
(et-defun et:algebra--case-expand-alias (case: *et-type-case alias: *et-alias) List<*et-type-case>
...
(et-defun et:algebra--intersect-datatypes (subsect?: Boolean a: *et-datatype b: *et-datatype)
          (or *et-datatype @INVALID)
  ...
  (let* ((a-name (et-datatype-name a))
    ...
    (cond
     ((et:algebra--dt-subtype? a b) a)
     ((et:algebra--dt-subtype? b a) b)
...
(et-defun et:algebra--subtract-cases (a-case: *et-type-case b-case *et-type-case) List<*et-type-case>
  (let* ((a (et-type-case-value a-case))
    ...
    (cond
     ...
     ((et-alias-p a) (cl-loop for exp-case in (et:algebra--case-expand-alias a-case a)
                              nconc (et:algebra--subtract-cases exp-case b-case)))
     ((et-alias-p b) (cl-loop for exp-case in (et:algebra--case-expand-alias b-case b)
  ...
  (cl-loop for case in (et-type-cases type)
           ...
           (et-copy-with
            ...
            (pcase val
              ...
              ((cl-struct et-datatype name args)
               (let ((new-args (et:dt-map-type-args name args #'et--remove-type-labels)))
                 (make-et-datatype :name name :args new-args)))
